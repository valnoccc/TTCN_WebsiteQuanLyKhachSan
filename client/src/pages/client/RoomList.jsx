...
import { Search, Filter, Users, Maximize, Bed, BedDouble, X } from 'lucide-react'; // Import thêm BedDouble
        ...
        <div className="bg-gray-50 min-h-screen py-8 font-sans">
            <div className="container mx-auto px-4">
                ...
                <div className="mb-6">
                    ...
                    <p className="text-sm text-gray-500 mt-1">Tìm thấy {rooms.length} phòng phù hợp</p>
                </div>
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="w-full lg:w-1/4">
                        <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 sticky top-24">
                            ...
                            {/* 2. Sức chứa (Thay cho Tiện nghi cũ) */}
                            <div className="mb-8">
                            ...
                            <div className="mb-8">
                                ...
                                <div className="space-y-3">
                                    {[
                                        ...
                                        { label: '2 Giường đôi (Gia đình)', icon: <BedDouble size={16} /> }
                                    ].map((item, index) => (
                                        <label key={index} className="flex items-center gap-3 cursor-pointer group">
                                            <input type="checkbox" className="w-5 h-5 rounded border-gray-300 text-teal-600 focus:ring-teal-500 transition" />
                                            <span className="text-gray-600 flex items-center gap-2 group-hover:text-teal-600 transition">
                                ...
                                <div key={room.MaPhong} className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg transition duration-300 group flex flex-col h-full">
                                    ...
                                    <div className="relative h-60 overflow-hidden">
                                        ...
                                        {room.TrangThai === 'Trong' ? (
                                            <span className="absolute top-4 right-4 bg-green-500 text-white text-xs font-bold px-3 py-1 rounded-full uppercase shadow-sm">
                                                Còn trống
                                            </span>
                                        ) : (
                                            <span className="absolute top-4 right-4 bg-red-500 text-white text-xs font-bold px-3 py-1 rounded-full uppercase shadow-sm">
                                                Đã đặt
                                            </span>
                                        )}
                                    </div>
                                    ...
                                    <div className="p-6 flex flex-col flex-grow">
                                        <div className="mb-4">
                                            ...
                                            <p className="text-gray-500 text-sm line-clamp-2">
                                                {room.MoTa || "Phòng nghỉ hiện đại với không gian thoáng đãng, đầy đủ tiện nghi, mang lại trải nghiệm thư giãn tuyệt vời."}
                                            </p>
                                        </div>
                                        ...
                                        <div className="flex flex-wrap gap-3 text-sm text-gray-500 mb-6">
                                            <span className="flex items-center gap-1.5 bg-gray-50 px-2.5 py-1.5 rounded-lg border border-gray-100">
                                                <Users size={16} className="text-teal-600" /> {room.SucChua} người
                                            </span>
                                            <span className="flex items-center gap-1.5 bg-gray-50 px-2.5 py-1.5 rounded-lg border border-gray-100">
                                                <Maximize size={16} className="text-teal-600" /> {room.DienTich}m²
                                            </span>
                                            {/* Giả lập loại giường dựa theo sức chứa */}
                                            <span className="flex items-center gap-1.5 bg-gray-50 px-2.5 py-1.5 rounded-lg border border-gray-100">
                                                <BedDouble size={16} className="text-teal-600" />
                                                {room.SucChua > 2 ? '2 Giường đôi' : '1 Giường lớn'}
                                            </span>
                                        </div>
                                        ...
                                        <div className="mt-auto flex items-center justify-between pt-4 border-t border-gray-100">
                                            ...
                                            <div className="flex gap-3">
                                                <Link
                                                    to={`/rooms/${room.MaPhong}`}
                                                    className="px-5 py-2.5 border-2 border-teal-600 text-teal-600 font-bold rounded-lg hover:bg-teal-50 transition text-sm"
                                                >
                                                    Chi tiết
                                                </Link>
                                                <button
                                                    disabled={room.TrangThai !== 'Trong'}
                                                    className={`px-5 py-2.5 font-bold rounded-lg transition text-sm shadow-md ${room.TrangThai === 'Trong'
                                                            ? 'bg-teal-600 text-white hover:bg-teal-700 shadow-teal-200'
                                                            : 'bg-gray-300 text-gray-500 cursor-not-allowed'
                                                        }`}
                                                >
                                                    {room.TrangThai === 'Trong' ? 'Đặt ngay' : 'Hết phòng'}
                                                </button>
                                            </div>
                                        </div>
                                    </div>
